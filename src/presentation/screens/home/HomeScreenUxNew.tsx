...
/* useEffect para escuchar posibles notificaciones desde el topic de SNS y estructurar los datos recibidos: */
  useEffect(() => {
    ...
    const unsubscribe = messaging().onMessage(async remoteMessage => {
      try {
        console.log("Mensaje recibido desde SNS:", remoteMessage.data);
        // Verificar si hay datos y si contienen la clave `default`
        if (remoteMessage.data?.default) {

         
          const rawData = remoteMessage.data.default;
          console.log("Contenido de `default`: rawData--->", rawData);

          // Primer análisis: Parsear `default` para obtener el JSON principal
          const data = JSON.parse(rawData);

          console.log("Datos parseados (nivel 1):", data);
